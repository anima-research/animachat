...
export class EnhancedInferenceService {
      ...
      if (isComplete) {
        ...
        if (onMetrics) {
          ...
          await onMetrics({
            ...
            details: this.buildUsageDetails(breakdown, inputTokens, outputTokens, cachedTokens, savings)
          });
        }
      }
    };
  ...
  private buildUsageDetails(
    ...
    if (inputTokens > 0) {
      details.input = this.createTokenUsage(breakdown.inputCost, inputTokens);
    }

    if (outputTokens > 0) {
      details.output = this.createTokenUsage(breakdown.outputCost, outputTokens);
    }

    if (cachedTokens > 0 || savings > 0) {
      details.cached_input = this.createTokenUsage(0, cachedTokens, savings > 0 ? -savings : 0);
    }
  ...
  private createTokenUsage(price: number, tokens: number, credits?: number): GrantTokenUsage {
    return {
      price,
      tokens,
      credits: credits === undefined ? price : credits
    };
  }
